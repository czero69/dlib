...
int main(int argc, char** argv) try
{
    ...
    const std::string trainingPath = argv[1];
    const std::string testingPath = argv[2];


    std::vector<matrix<rgb_pixel>> images_train, images_test;
    std::vector<std::vector<mmod_rect>> boxes_train, boxes_test;
    load_image_dataset(images_train, boxes_train, trainingPath);
    load_image_dataset(images_test,  boxes_test,  testingPath);
    ...
    //dummy test
    /*for( auto && i : images_train)
    {
        net(i); // it is ok;
    }

    net(images_train); // it is also ok - processing in batch
    */

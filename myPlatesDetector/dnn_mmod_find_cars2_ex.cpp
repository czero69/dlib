...
//using net_to_tranfer_type = loss_mmod<con<7,9,9,1,1,_55_rfireBlock5_v2<coord_downsampler2_v2<rcon1_1< 1 , input_rgb_image_pyramid<pyramid_down<4>>>>>>>;

using net_type = loss_mmod<con<1,9,9,1,1,rcon5<downsampler<input_rgb_image_pyramid<pyramid_down<4>>>>>>;
using net_to_transfer_type = loss_mmod_parts<4, con<1,9,9,1,1,rcon5<downsampler<input_rgb_image_pyramid<pyramid_down<4>>>>>>;



template < int N> void LAYERS_FOR_LOOP(net_to_transfer_type & nt, net_type & n);
template <>void LAYERS_FOR_LOOP<0>(net_to_transfer_type & nt, net_type & n) {};// note it is <0> here and not <-1>. It will not use layer<0> which is add_loss_layer
template < int N > void LAYERS_FOR_LOOP(net_to_transfer_type & nt, net_type & n) {
    LAYERS_FOR_LOOP<N-1>(nt, n);
    //layer<N>(nt).layer_details() = layer<N>(n).layer_details();
    layer<N>(nt).layer_details().setup(layer<N>(nt).subnet());
    layer<N>(nt) = layer<N>(n);
    }
...
int main(int argc, char** argv) try
{
    ...
    mmod_parts_options mmod_parts_options;
    ...
    std::cout << "before deserialize layer<7>(net).layer_details().get_layer_params().nr: " << layer<7>(net).layer_details().get_layer_params().nr() << std::endl;
    deserialize(detectorPath) >> net;
    std::cout << "after deserialize layer<7>(net).layer_details().get_layer_params().nr: " << layer<7>(net).layer_details().get_layer_params().nr() << std::endl;

    mmod_parts_options.bbr_lambda = net.loss_details().get_options().bbr_lambda;
    mmod_parts_options.overlaps_nms = net.loss_details().get_options().overlaps_nms;
    mmod_parts_options.overlaps_ignore = net.loss_details().get_options().overlaps_ignore;
    mmod_parts_options.assume_image_pyramid = net.loss_details().get_options().assume_image_pyramid;
    mmod_parts_options.loss_per_false_alarm = net.loss_details().get_options().loss_per_false_alarm;
    mmod_parts_options.loss_per_missed_target = net.loss_details().get_options().loss_per_missed_target;
    mmod_parts_options.truth_match_iou_threshold = net.loss_details().get_options().truth_match_iou_threshold;
    mmod_parts_options.use_bounding_box_regression = net.loss_details().get_options().use_bounding_box_regression;

    for(auto && a :  net.loss_details().get_options().detector_windows)
            mmod_parts_options.detector_windows.push_back(mmod_parts_options::detector_window_details(a.width, a.height, a.label));

    net_to_transfer_type net_to_transfer(mmod_parts_options);

    //std::cout << "layer<3>(net).layer_details().num_filters(): " << layer<3>(net).layer_details().num_filters() << std::endl;

    // check if layers copied correctly ...


    //std::cout << "layer<37>(net).layer_details().get_layer_params().host()[0]" << layer<37>(net).layer_details().get_layer_params().host()[0] << std::endl;

    // this is how to copy layer parameters
    //layer<31>(net_to_transfer).layer_details() = layer<37>(net).layer_details();


    //model 5 transfer experiment

    // unroll to 13..1
    //net.subnet().layer_details().set_num_filters(options.detector_windows.size() * 5);

    net_to_transfer.clean();
    layer<1>(net_to_transfer).layer_details().set_num_filters(35);
    // copy layer pyramid rgb input
    //layer<14>(net_to_transfer) = layer<14>(net);
    input_layer(net_to_transfer) = input_layer(net); //same as layer<14>(net_to_transfer) = layer<14>(net);
    // 13 here is next layer after input layer (14)
    // layer forward to (next to) rgb pyramid input cannt call setup over input layer pyramid rgb (no get_output(); defined)
    // so it has to be called here explicilty (outside the loop)
    layer<13>(net_to_transfer).layer_details() = layer<13>(net).layer_details();
    // loop over layers from A to B (B defined in loop definition), here integer range: <12,1>
    LAYERS_FOR_LOOP<12>(net_to_transfer, net);

    //layer<13>(net_to_transfer).layer_details() = layer<13>(net).layer_details();

    net_to_transfer.clean();
    // TODO: serialize network, remember about network.clean();


    cout << "The *net* has " << net.num_layers << " layers in it." << endl;
    cout << net << endl;

    cout << "The *net-to-transfer* has " << net_to_transfer.num_layers << " layers in it." << endl;
    cout << net_to_transfer << endl;


    std::cout << "layer<7>(net).layer_details().get_layer_params().nr: " << layer<7>(net).layer_details().get_layer_params().nr() << std::endl;
    std::cout << "layer<7>(net_to_transfer).layer_details().get_layer_params().nr: " << layer<7>(net_to_transfer).layer_details().nr() << std::endl;

    std::cout << "layer<7>(net).layer_details().get_layer_params().host()[0]: " << layer<7>(net).layer_details().get_layer_params().host()[0] << std::endl;
    std::cout << "layer<7>(net_to_transfer).layer_details().get_layer_params().host()[0]: " << layer<7>(net_to_transfer).layer_details().get_layer_params().host()[0] << std::endl;

    //exit(0);
    //layer<1>(net_to_transfer).layer_details() = layer<1>(net).layer_details();

    //std::cout << "layer<37>(net).layer_details().get_layer_params().host()[0]" << layer<37>(net).layer_details().get_layer_params().host()[0] << std::endl;
    //std::cout << "layer<31>(net_to_transfer).layer_details().get_layer_params().host()[0]" << layer<31>(net_to_transfer).layer_details().get_layer_params().host()[0] << std::endl;
    ...
    for (auto&& d : net_to_transfer(img))
    {
    ...
    std::cout << "after net(img) layer<7>(net).layer_details().get_layer_params().nr: " << layer<7>(net).layer_details().get_layer_params().nr() << std::endl;
    std::cout << "after net(img) layer<7>(net_to_transfer).layer_details().get_layer_params().nr: " << layer<7>(net_to_transfer).layer_details().nr() << std::endl;
    ...
    std::cout << "aw net.subnet().get_output().k() :" <<  net_to_transfer.subnet().get_output().k() << std::endl;
    std::cout << "aw net.subnet().layer_details().get_layer_params().k() :" <<  net_to_transfer.subnet().layer_details().get_layer_params().k() << std::endl;
    ...
    */
    ...
    for(int i = 0; i < NORMAL_TEST_COUNT; i++)
        net_to_transfer.process(img, -0.5); //net(img);
    ...
    using pyramid_type = std::remove_reference<decltype(input_layer(net_to_transfer))>::type::pyramid_type;
    ...
    create_tiled_pyramid<pyramid_type>(img, tiled_img, rects,
                                       input_layer(net_to_transfer).get_pyramid_padding(),
                                       input_layer(net_to_transfer).get_pyramid_outer_padding());
    ...
    cout << "Number of channels in final tensor image: " << net_to_transfer.subnet().get_output().k() << endl;
    matrix<float> network_output = image_plane(net_to_transfer.subnet().get_output(),0,0);
    matrix<float> network_output_huntingForRegression_k5 = image_plane(net_to_transfer.subnet().get_output(),0,7);
    for (long k = 1; k < net_to_transfer.subnet().get_output().k() / 5 ; k += 1)
        network_output = max_pointwise(network_output, image_plane(net_to_transfer.subnet().get_output(),0,k));
    ...
    for (long r = 0; r < tiled_img.nr(); ++r)
    {
        ...
        {
            dpoint tmp(c,r);
            tmp = input_tensor_to_output_tensor(net_to_transfer, tmp);
    ...
    input_layer(net_to_transfer).to_tensor(&img, &img+1, input_tensor);
    for (long r = 0; r < collapsed.nr(); ++r)
    {
        for (long c = 0; c < collapsed.nc(); ++c)
        {
            ...
            for (double scale = 1; scale > 0.2; scale *= 5.0/6.0)
            {
                ...
                dpoint tmp = center(input_layer(net_to_transfer).image_space_to_tensor_space(input_tensor,scale, drectangle(dpoint(c,r))));
                // Now map from pyramid coordinates to network_output coordinates.
                tmp = point(network_output_scale*input_tensor_to_output_tensor(net_to_transfer, tmp));

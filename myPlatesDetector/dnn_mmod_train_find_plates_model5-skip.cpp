...
// what I want to do here is splash image to percetul field wxh, so conv of size 1,hx3 will do the job, or avg
// I need to optain perceptual field somehow height and width

// what I want to do here is extend perceptual field beyond plate for plate region
// the idea is as follows: on one of the final layer, downsample image (lets say by 1/3 h by avraging but with proper padding instead of 0 padding)
// get coordinates of original layer and get according downasmpled coordinates, pool roi from downampled coordinates (by 1/3 h)
// then cocnatenate two same-sized layers, use concatenate to creat final 1-depth layer for sliding window mechanism

// QUESTION 1 how loss mmod layer knows, overall downsamplig is 8x

...
{
    std::array<matrix<dlib::rgb_pixel>, 2> input_image_combined;
...
void randomly_crop_image (
    const std::array<matrix<rgb_pixel>, 2>& input_image_combined,
    const matrix<uint16_t>& label_image,
    training_sample& crop,
    dlib::rand& rnd
)
{
    const auto rect = make_random_cropping_rect_resnet(input_image_combined[0], rnd);

    const chip_details chip_details(rect, chip_dims(227, 227));
    // Crop the input image.
    extract_image_chip(input_image_combined, chip_details, crop.input_image_combined, interpolate_bilinear());
    ...
    {
        crop.input_image_combined[0] = fliplr(crop.input_image_combined[0]); // flip should be identical
        crop.input_image_combined[1] = fliplr(crop.input_image_combined[1]); // flip should be identical
        crop.label_image = fliplr(crop.label_image); // flip should be identical
    }

    // And then randomly adjust the colors.
    apply_random_color_offset(crop.input_image_combined[0], rnd);
    apply_random_color_offset(crop.input_image_combined[1], rnd);
}
...
struct image_info
{
    string image_filename_foreground;
    string image_filename_background;
...
)
{
    ...
    while (in)
    {
        ...
        if (!basename.empty())
        {
            ...
            image_info.image_filename_background = voc2012_folder + "/JPEGImages/" + basename + "_bg" + ".jpg";
            image_info.image_filename_foreground = voc2012_folder + "/JPEGImages/" + basename + "_fg" + ".jpg";
...
double calculate_accuracy(anet_type& anet, const std::vector<image_info>& dataset)
{
    ...
    for (const auto& image_info : dataset)
    {
        ...
        load_image(input_image_combined[0], image_info.image_filename_background);
        load_image(input_image_combined[1], image_info.image_filename_foreground);
        ...
        const matrix<uint16_t> temp = anet(input_image_combined);
        ...
        const chip_details chip_details(
            centered_rect(input_image_combined[0].nc() / 2, input_image_combined[0].nr() / 2, input_image_combined[0].nc(), input_image_combined[0].nr()),
            chip_dims(input_image_combined[0].nr(), input_image_combined[0].nc())
        );
...
int main(int argc, char** argv) try
{
    ...
    std::vector<std::array<matrix<dlib::rgb_pixel>, 2>> samples;
    ...
    auto f = [&data, &listing](time_t seed)
    {
        ...
        std::array<matrix<rgb_pixel>, 2> input_image_combined;
        matrix<rgb_pixel> rgb_label_image;
        matrix<uint16_t> index_label_image;
        ...
        while(data.is_enabled())
        {
            ...
            randomly_crop_image(input_image_combined, index_label_image, temp, rnd);
    ...
    while(trainer.get_learning_rate() >= 1e-4)
    {
        ...
        while(samples.size() < 30)
        {
            ...
            samples.push_back(std::move(temp.input_image_combined));

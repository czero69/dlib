...
int main(int argc, char** argv) try
{
    ...
    for (const file& file : files)
    {
        ...
        load_image(input_image_combined[0], "path-to-bg-image");
        load_image(input_image_combined[1], file.full_name());

        // Create predictions for each pixel. At this point, the type of each prediction
        // is an index (a value between 0 and 20). Note that the net may return an image
        // that is not exactly the same size as the input.
        const matrix<uint16_t> temp = net(input_image_combined);

        // Crop the returned image to be exactly the same size as the input.
        const chip_details chip_details(
            centered_rect(temp.nc() / 2, temp.nr() / 2, input_image_combined[0].nc(), input_image_combined[0].nr()),
        ...
        //win.set_image(join_rows(input_image_combined[1], rgb_label_image));
        win.set_image(join_cols(join_rows(input_image_combined[0], rgb_label_image), join_rows(input_image_combined[1], rgb_label_image)));
